...
// Position-weighted specificity scoring rules (weight = totalSegments - resourceIndex):
//
//   totalSegments = total pattern segments
//   resourceIndex = 0-based segment index (left = higher weight)
//
//   Exact literal segment   : score += (totalSegments-resourceIndex) * 2
//   Wildcard segment        : score += (totalSegments-resourceIndex) * 1
//   Terminal wildcard       : scored once at its position; absorbed extras add nothing
//   Global "*" alone        : fixed score = 1 (lowest possible)
//
// Example (totalSegments=3, path "wallets/w1/transactions", weights 3,2,1):
...
class SpecificityScorerTest {
    ...
    class ExactMatches {
        ...
        @Test
        @DisplayName("Single exact segment: totalSegments=1, weight=1 → 1*2 = 2")
        void singleExactSegment() {
            assertThat(scorer.calculateScore("transactions", "transactions")).isEqualTo(2);
        }

        @Test
        @DisplayName("Two exact segments: totalSegments=2, weights 2,1 → 2*2 + 1*2 = 6")
        void twoExactSegments() {
            assertThat(scorer.calculateScore("wallets/wallet-789", "wallets/wallet-789")).isEqualTo(6);
        }

        @Test
        @DisplayName("Three exact segments: totalSegments=3, weights 3,2,1 → 3*2 + 2*2 + 1*2 = 12")
    ...
    class WildcardScoring {
        ...
        @Test
        @DisplayName("wallets/* vs direct child: totalSegments=2, weights 2,1 → 2*2 + 1*1 = 5")
        ...
        void terminalWildcard_nestedPath_sameScore() {
            ...
            // wallets/* (totalSegments=2): 2*2 + 1*1 = 5, regardless of how deep the path goes.
        ...
        @Test
        @DisplayName("wallets/wallet-789/* vs 3-seg path: totalSegments=3, weights 3,2,1 → 3*2 + 2*2 + 1*1 = 11")
        ...
        @Test
        @DisplayName("Non-terminal wildcard: wallets/*/transactions: totalSegments=3, weights 3,2,1 → 3*2 + 2*1 + 1*2 = 10")
        ...
        @Test
        @DisplayName("Mixed wildcards: wallets/*/transactions/*: totalSegments=4, weights 4,3,2,1 → 4*2 + 3*1 + 2*2 + 1*1 = 16")
    ...
    class TieBreaking {
        ...
        @Test
        @DisplayName("wallets/w1/*(11) beats wallets/*/transactions(10) — literal prefix wins over literal suffix")
        void literalPrefixWildcard_beatsWildcardPrefix_forSamePath() {
            // path: wallets/w1/transactions (totalSegments=3 for both patterns)
        ...
        void laterWildcard_beatsEarlierWildcard_sameDepth() {
            ...
            // wallets/w1/*           = 3*2 + 2*2 + 1*1 = 11  (wildcard at resourceIndex 2)
            // wallets/*/transactions = 3*2 + 2*1 + 1*2 = 10  (wildcard at resourceIndex 1)
            String path = "wallets/w1/transactions";
        ...
        void deeperTerminalWildcard_beatsShallowerTerminalWildcard() {
            // wallets/wallet-789/* (totalSegments=3): 3*2 + 2*2 + 1*1 = 11
            // wallets/*            (totalSegments=2): 2*2 + 1*1       =  5
